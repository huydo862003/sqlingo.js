...
export function schemaToDbml (schema: DbmlSchema): string {
  ...
  for (const table of schema.tables) {
    ...
    for (const column of table.columns) {
      ...
      if (column.default !== undefined) settings.push(`default: \`${column.default}\``);
      if (column.note) settings.push(`note: '${column.note}'`);
      if (column.check) settings.push(`check: \`${column.check.expression}\``);
      for (const ref of column.ref ?? []) settings.push(`ref: ${ref.relation} ${emitEndpoint(ref.target)}`);
      const st = settings.length ? ` [${settings.join(', ')}]` : '';

      lines.push(`  ${column.name} ${emitType(column.type)}${st}`);
    }
    // Generate indexes
    if (table.indexes?.length) {
      ...
      for (const index of table.indexes) {
        const columns = index.columns.map((column) => column.isExpression ? `\`${column.expression}\`` : column.expression);
        const columnsString = 1 < columns.length ? `(${columns.join(', ')})` : columns[0];
  ...
  for (const records of schema.records) {
    ...
    const columnList = records.columns.length ? `(${records.columns.join(', ')})` : '';

    lines.push(`records ${qualifiedName}${columnList} {`);

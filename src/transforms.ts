...
export function unnestGenerateDateArrayUsingRecursiveCte (expression: Expression): Expression {
  if (expression instanceof SelectExpr) {
    ...
    for (const unnest of expression.findAll(UnnestExpr)) {
      ...
      const columnName = aliasExpr instanceof TableAliasExpr ? aliasExpr.columns[0] : 'date_value';
    ...
    if (0 < recursiveCtes.length) {
      const withExpr = expression.args.with || new WithExpr({
        expressions: [],
      });
      withExpr.setArgKey('recursive', true);
      withExpr.setArgKey('expressions', [...recursiveCtes, ...(withExpr.expressions || [])]);
  ...
  if (expression instanceof TableExpr && thisArg instanceof GenerateSeriesExpr) {
    ...
    if (expression.args.alias) {
      return alias(
        ...
        '_u',
        {
          table: [expression.args.alias instanceof TableAliasExpr ? expression.args.alias.args.this || '' : expression.args.alias],
        },
      );
    }
    return unnest;
...
export function eliminateDistinctOn (expression: Expression): Expression {
  if (
    ...
    if (order) {
      ...
      window.setArgKey('order', new OrderExpr({ expressions: distinctCols?.map((c) => c.copy()) || [] }));
    }

    expression.select(alias(window, rowNumberAlias), { copy: false });

    let newSelects: (Expression | string | undefined)[] = [];
...
export function eliminateQualify (expression: Expression): Expression {
  if (expression instanceof SelectExpr && expression.args.qualify) {
    ...
    const selectAliasOrName = (s: Expression) => {
      ...
      const id = s.getArgKey('alias') || s.this;
    ...
    const expressionByAlias: Record<string, Expression> = {};
    for (const s of expression.selects) {
      if (s instanceof AliasExpr) expressionByAlias[s.alias] = s.args.this;
    }

    const candidates = qualifyFilters?.findAll<WindowExpr | ColumnExpr>(expression.isStar ? [WindowExpr] : [WindowExpr, ColumnExpr]);
    ...
    return outerSelects.from(expression.subquery('_t', { copy: false })).where(qualifyFilters);
  }
  return expression;
}
    ...
    if (0 < unnestAliases.size) {
      for (const column of expression.findAll(ColumnExpr)) {
        ...
        if (
          ...
          && unnestAliases.has(leftmostPart.this as string)
        ) {
          leftmostPart.pop();
        }
      }
...
export function unnestToExplode (
  ...
  if (expression instanceof SelectExpr) {
    ...
    if (from && from.args.this instanceof UnnestExpr) {
      ...
      const hasMultiExpr = 1 < exprs.length;
    ...
    for (const join of [...joins]) {
      ...
      if (unnest instanceof UnnestExpr) {
        ...
        const aliasCols = alias ? [...alias.columns] : [];
...
export function moveCtesToTopLevel<T extends SelectExpr> (expression: T): T {
  ...
  for (const innerWith of expression.findAll(WithExpr)) {
    ...
    if (!topLevelWith) {
      ...
      if (parentCte) {
        const index = topLevelWith.args.expressions.indexOf(parentCte);
        topLevelWith.args.expressions.splice(index, 0, ...innerExprs);
      } else {
        topLevelWith.setArgKey('expressions', [...topLevelWith.args.expressions, ...innerExprs]);
      }
    }
  }
  return expression;
  ...
  return (expression: Expression): Expression => {
    if (expression instanceof SelectExpr) {
      ...
      for (const select of [...expression.selects]) {
        ...
        if (explode) {
          ...
          if (explode instanceof ExplodeOuterExpr) {
            const bracket = explodeArg.expressions[0];
                ...
                expressions: [
                  new EqExpr({
                    this: new AnonymousExpr({
                      this: 'ARRAY_SIZE',
                      expressions: [new CoalesceExpr({ expressions: [explodeArg, new ArrayExpr({ expressions: [] })] })],
                    }),
          ...
          const size = new ArraySizeExpr({ this: explodeArg.copy() });
          arrays.push(size);
      ...
      if (0 < arrays.length) {
        ...
        (series.args.this as UnnestExpr).args.expressions[0].setArgKey('end', end);
      }
    }
    return expression;
  };
...
export function removeWithinGroupForPercentiles (expression: Expression): Expression {
  if (
    ...
    const quantile = expression.args.this.this;
    ...
    return expression.replace(new ApproxQuantileExpr({
      this: inputValue!,
      quantile: quantile as Expression,
    }));
  }
  return expression;
}
...
export function addRecursiveCteColumnNames (expression: Expression): Expression {
  ...
  if (expression instanceof WithExpr && expression.recursive) {
    ...
    for (const cte of expression.args.expressions) {
      if (!cte.args.alias?.columns) {
        let query = cte.args.this;
        if (query instanceof SetOperationExpr) {
          query = query.args.this;
        }

        cte.args.alias?.setArgKey(
          'columns',
          query.selects.map((s) =>
...
export function epochCastToTs (expression: Expression): Expression {
  if (
    ...
    && DataTypeExpr.TEMPORAL_TYPES.has(enumFromString(DataTypeExprKind, expression.args.to.args.this.toString())!)
  ) {
    expression.args.this.replace(LiteralExpr.string('1970-01-01 00:00:00'));
  }
  ...
  if (expression instanceof SelectExpr) {
    ...
    if (fullOuterJoins.length === 1) {
      ...
      const joinConditions = fullOuterJoin.args.on || new AndExpr({
        expressions: (fullOuterJoin.args.using || []).map((col) =>
          new EqExpr({
            this: new ColumnExpr({
              this: col,
              table: fromName,
      ...
      return union([expression, expressionCopy], { distinct: false });
    }
  }
...
export function moveSchemaColumnsToPartitionedBy (expression: Expression): Expression {
  ...
  if (hasSchema && isPartitionable) {
    ...
    if (prop && prop.args.this && !(prop.args.this instanceof SchemaExpr)) {
      ...
      const columns = new Set(prop.args.this.expressions.map((v) => v instanceof Expression ? v.name.toUpperCase() : v.toString()));
      ...
      prop.replace(new PartitionedByPropertyExpr({
        this: new SchemaExpr({ expressions: partitions }),
      }));
...
export function movePartitionedByToSchemaColumns (expression: Expression): Expression {
  ...
  if (
    ...
    const propThis = new TupleExpr({
      expressions: prop.args.this.args.expressions.map((e) => toIdentifier((e as ColumnDefExpr).args.this)),
    });
...
export function eliminateJoinMarks (expression: Expression): Expression {
  // Note: traverseScope and normalize utilities must be ported for this to function
  for (const scope of [...traverseScope(expression)].reverse()) {
    ...
    for (const j of joins) oldJoins[j.aliasOrName] = j;
      ...
      for (const struct of expression.args.expressions.slice(1)) {
        ...
        const newExpressions = struct.args.expressions.map((expr, i) => {
          if (!(expr instanceof PropertyEqExpr)) {
            const thisFieldName = fieldNames[i]?.this;

...
describe('TestBuild', () => {
  it('test_build', () => {
    ...
    const testCases: Array<[() => Expression | undefined, string, string?]> = [
      ...
      [
        () => LiteralExpr.number(1).floorDiv(x),
        'CAST(1 / x AS INT)',
      ],
      [
        () => x.mod(1),
      ...
      [
        () => LiteralExpr.number(1).add(x.mul(2).div(3)),
        '1 + ((x * 2) / 3)',
      ],
      [
        () => x.and('\'y\''),
      ...
      [
        () => select([
          ...
          .join('vte_diagnosis_df', {
            using: [
              toIdentifier('patient_id'),
              toIdentifier('encounter_id'),
            ],
          }),
        'SELECT x, y, z FROM merged_df JOIN vte_diagnosis_df USING (patient_id, encounter_id)',
      ],
      ...
      [
        () => subquery('select x from tbl UNION select x from bar', 'unioned').select('x'),
      ...
      [
        () => update('my_table', {
          ...
          with: {
            baz: 'SELECT id FROM foo UNION SELECT id FROM bar',
          },
        }),
      ...
      [
        () => alias(parseOne('LAG(x) OVER (PARTITION BY y)'), 'a'),
        'LAG(x) OVER (PARTITION BY y) AS a',
      ],
      [
        () => alias(parseOne('LAG(x) OVER (ORDER BY z)'), 'a'),
        'LAG(x) OVER (ORDER BY z) AS a',
      ],
      [
        () => alias(parseOne('LAG(x) OVER (PARTITION BY y ORDER BY z)'), 'a'),
        'LAG(x) OVER (PARTITION BY y ORDER BY z) AS a',
      ],
      [
        () => alias(parseOne('LAG(x) OVER ()'), 'a'),
        'LAG(x) OVER () AS a',
      ],
      [
        () => values([
      ...
      [
        () => cast('CAST(x AS INT)', 'int'),
        'CAST(x AS INT)',
      ],
      [
        () => cast('CAST(x AS TEXT)', 'int'),
        'CAST(CAST(x AS TEXT) AS INT)',
      ],
      [
        () => renameColumn('table1', 'c1', 'c2', {
          exists: true,
        }),

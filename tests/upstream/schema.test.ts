...
class TestSchema {
  ...
  testSchemaGetColumnType () {
    ...
    expect(schema1.getColumnType(table_('a'), column({
      col: 'b',
    }) as ColumnExpr)?.args.this).toBe(DataTypeExprKind.VARCHAR);
    expect(schema1.getColumnType('a', column({
      col: 'b',
    }) as ColumnExpr)?.args.this).toBe(DataTypeExprKind.VARCHAR);
    ...
    expect(schema2.getColumnType(table_('b', {
      db: 'a',
    }), 'c')?.args.this).toBe(DataTypeExprKind.VARCHAR);
  ...
  testSchemaNormalization () {
    ...
    expect(schema1.columnNames(tableZ)).toEqual([
      'a',
      'B',
    ]);
    ...
    expect(schema3.columnNames(table_('fOO'))).toEqual([
      'somecolumn',
      'SomeColumn',
    ]);
    ...
    const schema5 = new MappingSchema({
      x: {
        foo: 'int',
      },
    }, {
    ...
    const schema7 = new MappingSchema({
      Foo: {
        '`BaR`': 'int',
      },
    }, {
    ...
    const schemaNestedStruct = new MappingSchema(
      {
        t: {
          col: 'STRUCT<Outer STRUCT<Inner INT>>',
        },
      },
      {
        dialect: 'bigquery',
